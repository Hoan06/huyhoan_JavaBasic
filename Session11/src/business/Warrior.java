...
public class Warrior extends GameCharacter implements ISkill {
    ...
    public void attack(GameCharacter target) {
        ...
        System.out.printf("Chiến binh %s tấn công chiến binh %s . \n" , super.getName() , target.getName());
        System.out.printf("=> Chiến binh %s mất %d máu . Hp còn %d .\n" , target.getName() , super.getAttackPower() , target.getHp());
    }

    @Override
    public void useUltimate(GameCharacter target) {
        if (super.getHp() <= 10){
            System.out.println("Không đủ hp để sử dụng !.");
        } else {
            target.takeDamage(super.getAttackPower() * 2);
            super.setHp((int) (super.getHp() * 0.9));
            System.out.printf("Chiến binh %s sử dụng chiêu cuối vào %s !.\n", super.getName(), target.getName());
            System.out.printf("Chiến binh %s mất %d máu !.\n", target.getName(), super.getAttackPower() * 2);
        }
    }
    ...
    public void displayInfo() {
        System.out.printf("| Name : %s | Hp : %d | Giáp : %d |\n" , super.getName() , super.getHp() , this.armor );
    }
}
